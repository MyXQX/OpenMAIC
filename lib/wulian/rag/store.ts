 ...
 * - 上传资料：data/wulian/vectors/<docId>.json
 *
 * 检索：embedding 存在时余弦排序；不存在时关键词打分。
 */

import path from 'node:path';
import { createLogger } from '@/lib/logger';
import type { DocumentChunk, RetrievedChunk } from '@/lib/wulian/types';
import { CHAPTERS_DIR, KNOWLEDGE_DIR, VECTORS_DIR, readJson, writeJson } from '@/lib/wulian/storage';
